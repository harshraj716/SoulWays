...
class Db2QueryGenerator extends import_query_generator_typescript.Db2QueryGeneratorTypeScript {
      ...
      if (Object.hasOwn(attributes, attr)) {
        ...
        if (dataType.includes("COMMENT ")) {
          ...
          if (commentMatch && commentMatch.length > 2) {
            ...
            commentStr += (0, import_template.default)(
              commentTemplate,
  ...
  addColumnQuery(table, key, dataType, options) {
    ...
    const attribute = (0, import_template.default)(
      "<%= key %> <%= definition %>",
      this._templateSettings
    )({
    ...
    if (allAttributes.length > 0) {
      (0, import_forEach.default)(attrValueHashes, (attrValueHash) => {
        tuples.push(
          ...
          allAttributes.map(
            (key) => this.escape(attrValueHash[key] ?? null, { replacements: options.replacements })
          ).join(",")})`
        );
      });
  ...
  upsertQuery(tableName, insertValues, updateValues, where, model, options) {
    ...
    const filteredUpdateClauses = updateKeys.filter((key) => {
      ...
      const value = this.escape(updateValues[key], void 0, {
        replacements: options.replacements
      });
      key = this.quoteIdentifier(key);
