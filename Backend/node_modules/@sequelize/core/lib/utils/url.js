...
function parseConnectionString(connectionString) {
  ...
  if (urlObject.protocol) {
    ...
    if (!import_sequelize_typescript.SUPPORTED_DIALECTS.includes(protocol)) {
      throw new Error(
        `The protocol was set to ${JSON.stringify(protocol)}, which is not a supported dialect. Set it to one of ${import_sequelize_typescript.SUPPORTED_DIALECTS.map((d) => JSON.stringify(d)).join(", ")} instead.`
      );
    }
